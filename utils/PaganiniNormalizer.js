...
export default class PaganiniNormalizer {
	...
	/**
	 * Check if a product is a Leupold optic instead of an accessory.
	 * @returns {boolean} True if product title looks like an optic
	 */
	isOpticsProduct() {
		const title = (this.productData.Anagrafica_Paganini || "")
			.toString()
			.toUpperCase();
		...
		return (
			opticPatterns.some((pattern) => pattern.test(title)) &&
			!accessoryPatterns.some((pattern) => pattern.test(title))
		);
	}
	...
	shouldImport() {
		...
		const productCode = (this.productData.Codice_Produttore || "")
			.toString()
			.trim();
		...
		const productFilter = (this.config.paganiniProductFilter || "optics")
			.toString()
			.trim()
			.toLowerCase();
		if (productFilter === "all" || productFilter === "leupold") {
			return true;
		}

		if (productFilter === "optics") {
			return this.isOpticsProduct();
		}
	...
	generateSKU() {
		...
		if (!vendor || !productCode) {
			this.log(
				`Warning: Missing vendor or product code for SKU generation`,
				"WARN",
			);
			return null;
		}
	...
	normalizeDescription() {
		let description = (this.productData.Descrittivo_Paganini || "")
			.toString()
			.trim();
	...
	normalizePrice(priceInput) {
		...
		let cleanPrice = priceStr
			.replace(/[€$£¥₹₽₴₪₦₡₵₸₻₼₾₿]/g, "")
			.replace(
				/\b(EUR|USD|GBP|JPY|CNY|CAD|AUD|CHF|SEK|NOK|DKK|PLN|CZK|HUF|RUB|TRY|ILS|INR|BRL|MXN|KRW|ZAR|SGD|HKD|NZD|THB|MYR|IDR|PHP|VND)\b/gi,
				"",
			)
			.replace(/\s+/g, "")
			.trim();
		...
		if (Number.isNaN(numericPrice) || numericPrice < 0) {
			this.log(
				`Warning: Invalid price value "${priceStr}" could not be normalized`,
				"WARN",
			);
			return null;
		}
	...
	normalizeAvailability() {
		const availability = (this.productData.Disponibilita || "")
			.toString()
			.trim()
			.toUpperCase();
