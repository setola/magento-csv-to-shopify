...
async function createShopifyProduct(productData) {
  ...
  `;
	...
	try {
		...
		log(
			`✓ Created product: ${productData.title} (SKU: ${productData.sku})`,
			"SUCCESS",
		);

		// Update the default variant with SKU and price - this returns the new variant ID
		const newVariantId = await updateVariantDetails(
			product.id,
			null,
			productData,
		);
		...
		log(
			`✗ Failed to create product ${productData.sku}: ${error.message}`,
			"ERROR",
		);
		throw error;
	}
 ...
 * @param {Object} productData - Normalized product data
 */
async function updateVariantDetails(productId, _variantId, productData) {
	const mutation = `
  ...
  `;
	...
	try {
		...
		if (result.productVariantsBulkCreate?.userErrors?.length > 0) {
			throw new Error(
				JSON.stringify(result.productVariantsBulkCreate.userErrors),
			);
		}

		const newVariantId =
			result.productVariantsBulkCreate?.productVariants?.[0]?.id;
		log(
			`  ↳ Variant updated with SKU: ${productData.sku}, Price: ${productData.price}`,
			"DEBUG",
		);
...
async function updateShopifyProduct(productId, variantId, productData) {
  ...
  `;
	...
	try {
		...
		const productResult = await shopifyClient.query(productMutation, {
			input: productInput,
		});
		...
		log(
			`↻ Updated product: ${productData.title} (SKU: ${productData.sku})`,
			"SUCCESS",
		);
		...
		log(
			`✗ Failed to update product ${productData.sku}: ${error.message}`,
			"ERROR",
		);
		throw error;
	}
...
async function updateInventoryQuantity(variantId, quantity) {
  ...
  `;
    ...
    `;
		...
		log(
			`  ↳ Inventory set to ${quantity} units at ${CONFIG.paganiniLocationName}`,
			"DEBUG",
		);
	} catch (error) {
...
async function processProduct(rowData, index) {
	try {
		...
		if (!normalizer.shouldImport()) {
			log(
				`↷ Skipping row ${index + 1}: Not a LEUPOLD product (${rowData.Produttore})`,
				"DEBUG",
			);
			return { skipped: true };
		}
		...
		try {
			...
			log(
				`✗ Row ${index + 1}: Failed to normalize data - ${error.message}`,
				"ERROR",
			);
		...
		log(
			`Processing product ${index + 1}: ${productData.title} (SKU: ${productData.sku})`,
			"INFO",
		);

		// Check if product already exists
		const existingProduct = await shopifyClient.findProductBySku(
			productData.sku,
		);
...
async function main() {
	...
	try {
		...
		const tasks = batch.map(({ row, index }) => async () => {
			const result = await processProduct(row, index);
		...
		await rateLimiter.executeTasks(tasks, (completed, total, _error) => {
			if (completed % 5 === 0 || completed === total) {
				...
				log(
					`Progress: ${completed}/${total} (${stats.created} created, ${stats.updated} updated, ${stats.skipped} skipped) | Lap: ${timingStats.lapTimeFormatted} (${timingStats.avgTimePerItem}ms/product) | Total: ${timingStats.totalElapsedFormatted}`,
					"INFO",
				);
