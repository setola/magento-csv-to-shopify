...
async function shopifyGraphQL(query, variables = {}) {
  const url = `https://${CONFIG.shopifyStore}/admin/api/2024-10/graphql.json`;
  ...
  try {
    ...
    if (cost) {
      log(`GraphQL Cost: ${cost.actualQueryCost}/${cost.throttleStatus.currentlyAvailable}`, 'DEBUG');
      
      // Se stiamo per superare il limite, aspettiamo
      if (cost.throttleStatus.currentlyAvailable < 100) {
        const waitTime = 2000;
        log(`Rate limit approaching, waiting ${waitTime}ms`, 'WARN');
...
async function findProductBySku(sku) {
  ...
  try {
    ...
    log(`Error searching for SKU ${sku}: ${error.message}`, 'WARN');
    return null;
  }
}
...
// Parsing CSV Magento
function parseMagentoCSV(csvPath) {
...
async function migrateProducts() {
  ...
  try {
    ...
    // Seleziona batch
    const endRow = CONFIG.startRow + CONFIG.batchSize;
    const productsToMigrate = allProducts.slice(CONFIG.startRow, endRow);
    log(`Migrating rows ${CONFIG.startRow} to ${endRow} (${productsToMigrate.length} products)`);
    ...
    // Migra prodotti con rate limiting
    const tasks = productsToMigrate.map((product, idx) => 
      limiter(async () => {
        try {
          ...
          stats.failed++;
          log(`Failed processing row ${CONFIG.startRow + idx}: ${error.message}`, 'ERROR');
        }
