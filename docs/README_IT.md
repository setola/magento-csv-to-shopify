...
Lo script gestisce intelligentemente i prezzi e prezzi speciali da Magento:

### Comportamento Prezzi
- **Se `special_price` esiste ed è minore di `price`**: 
  - `special_price` → Prezzo principale Shopify (`variants[0].price`)
  - `price` → "Era" prezzo (`variants[0].compareAtPrice`)
- **Altrimenti**: 
  - `price` → Prezzo principale Shopify
  - Nessun "era" prezzo

### Esempi
| Magento | Risultato Shopify |
|---------|------------------|
| `price: 100.00`, `special_price: 80.00` | Prezzo: €80.00, Era: €100.00 |
| `price: 100.00`, `special_price: 120.00` | Prezzo: €100.00 (special_price ignorato) |
| `price: 100.00`, no special_price | Prezzo: €100.00 |
...
|| `special_price` | `variants[0].compareAtPrice` | Prezzo "era" con normalizzazione robusta |
...
3. **Categorie**: Le categorie Magento diventano tags (Shopify usa Collections)
4. **Costo prodotto**: Viene estratto da `additional_attributes.cost` se presente
...
- ✅ **Variante**: SKU, prezzo, prezzo "era" (compareAtPrice), costo (se presente)
